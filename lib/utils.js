...
const trimQuote = text => trimSymmetric(trimSymmetric(text.trim(), `'`), `"`).trim()
 ...
 */
function rawParse (cmd) {
  validate(cmd)
  cmd = trimEmptyLine(trimQuote(cmd.trim()))
  const result = []
  let param = ''
  ...
  result.push(param.trim())
  return result.filter(noEmpty).filter(noLeadingCurl).map(trimQuote)
}
