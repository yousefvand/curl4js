...
const process = args => config => {
  const c = utils.pipe(utils.rawParse, curlParse)(args)
  return utils.pipe(processFlags(c.flags), processSwitches(c.switches), processUrl(c.url))(config)
}
